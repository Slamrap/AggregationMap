...
def getCoordsScale_freguesia(db, distrito, concelho):
    query = "select min(st_y(st_astext((g).geom))) as min_Y, max(st_y(st_astext((g).geom))) as max_Y, " \
            "min(st_x(st_astext((g).geom))) as min_X, max(st_x(st_astext((g).geom))) as max_X " \
            "from (select st_dumpPoints(geom) as g from caop where " \
            "distrito like '" + distrito + "' and concelho like '" + concelho + "') as a;"
    result = db_query(db, query)
    return result
...
def help():
    ...
    print("   database.getCoordsScale_freguesia(db, distrito, concelho)")
